...
const SocialsBanner = () => {
	const socials = [
		{
			...
			icon: (
				<FaFacebookF
					size={20}
					className="text-blue-500 group-hover:text-accent duration-300"
				/>
			),
	...
	return (
		<section className="bg-blue-500 flex flex-col justify-center items-center h-[250px] w-full">
			...
			<div className="grid grid-cols-2 sm:grid-cols-4 gap-7 mt-5">
				{socials.map((social) => (
					<div
						key={social.name}
						className="group bg-white p-5 rounded-full hover:bg-white duration-300 cursor-pointer"
					>
						{social.icon}
